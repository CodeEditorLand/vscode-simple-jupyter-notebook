...
import { DisposableList, IDisposable, toDisposable } from "./disposable";
...
export class KernelProcess implements IDisposable {
	...
	public connectToProcessStdio(): IDisposable {
		return new DisposableList([
			toDisposable(
				this.stderr.subscribe((line) =>
					process.stderr.write(`kernel stderr> ${line}\n`),
				),
			),
			toDisposable(
				this.stdout.subscribe((line) =>
					process.stdout.write(`kernel stdout> ${line}\n`),
				),
			),
			toDisposable(
				this.exit.subscribe((err) =>
					err ? console.error(err.stack) : `Kernel exited gracefully`,
				),
			),
		]);
	}
