...
import { concat, filter, ignoreElements } from "rxjs/operators";
...
const fromRawMessage = <MT extends OriginalMessageType, C = unknown>(
	...
	({
		...rawMessage,
		channel,
		buffers: rawMessage.buffers
			? Buffer.concat(rawMessage.buffers)
			: undefined,
	}) as unknown as TypedJupyerMessage;

const toRawMessage = (
	rawMessage: TypedJupyerMessage,
): wireProtocol.RawJupyterMessage => {
	return {
		...rawMessage,
		header: rawMessage.header as JupyterMessageHeader<never>,
		parent_header:
			rawMessage.parent_header as JupyterMessageHeader<OriginalMessageType>,
...
export class Connection implements IDisposable {
	...
	public sendAndReceive(
		message: TypedJupyerMessage,
	): Observable<TypedJupyerMessage> {
		return from(this.sendRaw(message)).pipe(
			ignoreElements(),
			concat(this.messages),
			filter(
				(msg) => msg.parent_header?.msg_id === message.header.msg_id,
			),
		);
	}
...
async function createSocket<T extends zmq.Socket>(
	socket: T,
