...
export class KernelProvider {
	...
	public static defaultSearchPaths() {
		...
		if (process.platform === "win32") {
			searchPaths.push(
				{
					type: LocationType.User,
					path: `${process.env.APPDATA}\\jupyter\\kernels`,
				},
				{
					type: LocationType.Global,
					path: `${process.env.PROGRAMDATA}\\jupyter\\kernels`,
				}
			);
		} else {
			searchPaths.push(
				{
					type: LocationType.User,
					path: `${homedir()}/Library/Jupyter/kernels`,
				},
				{
					type: LocationType.User,
					path: `${homedir()}/.local/share/jupyter/kernels`,
				},
				{
					type: LocationType.User,
					path: `${homedir()}/opt/miniconda3/share/jupyter/kernels`,
				},
				{
					type: LocationType.User,
				...
				{
					type: LocationType.User,
					path: `/opt/conda/local/share/jupyter/kernels`,
				},
				{
					type: LocationType.Global,
	...
	constructor(
		private readonly searchPaths: () => ReadonlyArray<IKernelSpecSearchPath>
	) {}
				...
				if (await exists(jsonPath)) {
					...
					specs.push({
						...
						iconDataUri: (await exists(iconPath))
							? `image/png;base64,${await fs.readFile(
									iconPath,
									"base64"
							  )}`
							: undefined,
	...
	public async launchKernel(spec: IKernelSpec): Promise<IRunningKernel> {
		...
		const process = new KernelProcess(
			spawn(
				spec.binary,
				spec.argv.map((arg) =>
					arg.replace("{connection_file}", connection.connectionFile)
				),
				{ stdio: "pipe" }
			)
		);
