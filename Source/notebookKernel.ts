...
import { filter, reduce, takeUntil, takeWhile } from "rxjs/operators";
...
export class NotebookKernel implements vscode.NotebookKernel {
	...
	async resolve() {
		...
		this._resolveKernel = this.provider
			...
			.then(async (instance): Promise<IRunningKernel | undefined> => {
	...
	private async _executeCell(
		...
		const outputStream = kernel.connection
			.sendAndReceive(executeRequest(cell.document.getText()))
			.pipe(
				takeWhile(
					(msg) => msg.header.msg_type !== "execute_reply",
					true,
				),
				takeUntil(
					this._requestCancellation.pipe(
						filter(
							(r) => r.document === document || r.cell === cell,
						),
					),
				),
			);
